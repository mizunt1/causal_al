...
from scm_class import piif_complex_mech, piif_overlap_mech
...
def main(tc):
    ...
    if tc.data == 'piif':
        num_samples_1 = tc.prop1
        num_samples_2 = tc.prop2
        data_class = piif_complex_mech(
            device,  num_samples_1, num_samples_2)    
        data_train, target_train = data_class.generate_one_env(
            tc.seed, mech_type=1)
        data_train2, target_train2 = data_class.generate_one_env(
            tc.seed + 1, mech_type=2)
        data_test, target_test = data_class.generate_one_env(
            tc.seed + 2, mech_type=3)
        # must create a test set that is OOD, such that spurious mechansism
        # has never been seen in trainset. 
        env_train, env_test = data_class.return_envs(
            (data_train, data_train2), (target_train, target_train2),
            (data_test,),
            (target_test,))
    if tc.data == 'piif_overlap':
        num_samples_1 = tc.prop1
        num_samples_2 = tc.prop2
        data_class = piif_overlap_mech(
            device,  num_samples_1, num_samples_2)    
        data_train, target_train = data_class.generate_one_env(
            tc.seed, mech_type=1)
        data_train2, target_train2 = data_class.generate_one_env(
...
if __name__ == "__main__":
    ...
    column_names = ["train ac w",
                    "test acc", "test acc c mskd", "test acc sp mskd"]
    ...
    df1 = pd.DataFrame({"train ac w":train_acc_w.detach().cpu().item(),
                        ...
                        "test acc sp mskd": test_sp_masked.detach().cpu().item()}, index=[0])
    df = pd.concat([df, df1])
    # df for printing
    pretty_print("train ac w",
                 "test acc", "t/ac c mskd", "t/ac sp mskd")
    pretty_print(
